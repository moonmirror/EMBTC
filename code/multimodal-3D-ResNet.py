...
class GetLoader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_paths.append('img_t1_path')
        img_paths.append('img_t2_path')
        img_paths.append('img_t1c_path')
        img_paths.append('img_flair_path')

        img_data = torch.zeros((4, 128, 160, 160))
        ...
        for i in range(len(img_paths)):
            if img_paths[i] == 'img_t1_path':
                img_list = os.listdir(img_t1_path)
            elif img_paths[i] == 'img_t2_path':
                img_list = os.listdir(img_t2_path)
            elif img_paths[i] == 'img_t1c_path':
                img_list = os.listdir(img_t1c_path)
            else:
                img_list = os.listdir(img_flair_path)
            ...
            for j in ll:
                imgstr = str(j) + '.png'
                img = Image.open(os.path.join(str(self.pic_files[index]), imgstr))
                img = self.transform(img)
                image[j, :, :] = img

            img_data[i] = image[13:141, :, :]
...
def train():
    ...
    train_data = GetLoader('./T1_segment', 'train')
    train_loader = DataLoader(train_data, batch_size=8, shuffle=False, pin_memory=False)
    print('train.shape: ' + str(len(train_data)))


    test_data = GetLoader('./T1_segment', 'test')
    ...
    for i_epoch in range(1, epoch+1):
        ...
        for step, data in tqdm(enumerate(train_loader)):
            ...
            for i in range(len(ac)):
                if target_y[i] == 0 and ac[i] == 0:
                    acc_2 += 1
                elif target_y[i] == 1 and ac[i] == 0:
                    acc_4 += 1
        ...
        fig = plot_confusion_matrix(conf_matrix_train.numpy(), classes=['LGG', 'HGG'])
        ...
        loss_test, confmatrix, fpr, tpr, roc_auc, SEN, SPE, acc_sum, acc_all = test(test_loader, path_ckpt, dict_test,
                                                                                    dict_label_test)
        test_acc = acc_sum * 100
        test_acc_all = acc_all * 100

        print("[Epoch " + str(i_epoch) + " | " + "test loss = " + ("%.7f" % loss_test) + ", test mAP = " + (
                    "%.2f" % test_acc) + \
              "%, test acc = " + ("%.2f" % test_acc_all) + "%, test auc = " + ("%.4f" % roc_auc) + ", test SEN = " + (
                          "%.4f" % SEN) \
              + ", test SPE = " + ("%.4f" % SPE) + "]")
        ...
        fig = plot_confusion_matrix(confmatrix.numpy(), classes=['LGG', 'HGG'])
